...
### September 26, 2026: LoRa link + AWS fallback, INA226 probe, Battery SoC redesign, charger removal

Ported from the standalone (plain-MQTT) working project into this AWS IoT
Core version. **Everything the firmware team marked off-limits is untouched**
(`aws_manager.c/h`, `gsm_mqtt.c/h`, `device_config.h`, AWS_Init()'s body and
arguments, topics, payloads, certificates, flash sector 6). All additions live
in main.c, screen_sm.c, hmi_state, battery_soc, ina226 and new files.

**Supersedes the "Charger + Battery SoC" section above.** That section
describes the on-board charger detection (PA6/PA7 ADC + TIM3 + PA5 GSM power
cut), which no longer exists — the pack is charged externally.

- **LoRa is the preferred link, AWS/GSM the fallback** (`lora_e220.c/h`, new).
  E220 on USART6 (PA11/PA12), M1/M0 = PA1/PA4 held LOW (Normal mode, no boot
  configuration — a boot-time config write was the likely cause of the
  standalone project's module resetting and intermittent link loss).
  Frame `*D<NN>:<cm>\r\n` (centimetres, so LoRa and AWS both feed
  `g_hmi.distance_cm` in the same unit). Rolling 5 x 1.5s window: STABLE at
  >=4 hits, back to LISTENING at <=2. `screen_sm.c` owns the handover:
  `ConnectionIsReady()` waits for LoRa (3s no-signal grace, 10s while LoRa is
  merely present), then falls back to AWS telemetry; `TickDistanceFromLinks()`
  reads whichever is authoritative (`g_hmi.conn_mode`); the status bar shows
  LORA/GSM accordingly; the buzzer/obstacle logic only treats the link as down
  when BOTH are down. Pairing presence = GSM presence OR LoRa presence.
  `STARTUP_HOLD_MS` is 5000 (was 3000) — matches the proven standalone value.
- **AWS_Init() is retried from the main loop, never blocked on**
  (MASTER PROMPT 9.2). Same call, same arguments. Attempts start only after
  the Startup screen ends, are skipped while a LoRa session is live
  (LISTENING/STABLE), and back off 5s -> 10s -> ... -> 60s after each failure.
  `LoRa_Poll()` runs first in every loop iteration.
- **SIM check** (replaces the standalone `fullReconnect()` fix, which cannot be
  ported because `gsm_mqtt.c` is protected): `modem_gate.c` sends plain `AT` +
  `AT+CPIN?` (~2s worst case) before each `AWS_Init()` attempt and skips the
  attempt when the modem is silent or no SIM is present. Uses only the public
  `GSM_SetUART()`/`GSM_SendAT()`.
- **INA226 address probe** (`ina226.c`): scans 0x40-0x4F for the manufacturer
  ID (0x5449) — the fitted module answers on 0x44, not the default 0x40 —
  plus I2C bus-clear and finite timeouts. Root cause of "battery always 0%/50%".
- **Battery SoC redesign** (`battery_soc.c`): discharge-only, 10% buckets,
  3s boot voltage read, 5s hold before a bucket drops, 12.0V = 100%, 3S Li-ion
  (safety floor 9.0V), 4000mAh Coulomb counting persisted in flash **sector 7
  (0x08060000)** with recharge detection on boot (saved vs. boot voltage) so a
  reboot no longer resets a discharged pack to 90%. Sector 6 (AWS credentials)
  is never touched.
- **Charger removal**: `charger_detect.c/h` deleted, TIM3/ADC1 left initialised
  but unused (regenerated by CubeMX anyway), `SCR_13_CHARGER_PLUGGED` enum
  entry kept (unused) because `screen_id_t` maps 1:1 onto DGUS page IDs.
  `g_hmi.charger_plugged` is kept and is permanently `false` — `aws_manager.c`
  still reads it (with `distance_cm`, `connected_device_num`, `battery_pct`).
- **Hardware notes**: PA5 (MOSFET feeding the E220 and the GSM modem) must stay
  HIGH; never drive it low while LoRa is needed. Powering the board from a USB
  charger/power bank shorts USART6 TX to RX through PA11/PA12 (seen on the
  standalone unit) — use the battery for LoRa testing.
- `.ioc` updated (USART6, PA11/PA12, NVIC, PA1/PA4 outputs). Local build
  verified with `arm-none-eabi-gcc` (no warnings, ~63 KB flash).

#### Pre-flash audit follow-ups (same day)

- **Telemetry distance readout**: shown in metres — whole metres from 10 m up
  ("23"), one decimal below 10 m in half-metre steps only ("9.5", "9.0",
  "8.5"), rounded to the nearest step (`FormatDistanceText()` in
  `screen_sm.c`). Display only: `g_hmi.distance_cm`, the buzzer bands and the
  obstacle detection still work from the exact centimetre value. "OR" (> 45 m)
  and "DC" (both links down) are unchanged.
- **Cloud reconnect no longer freezes the loop**: `GSM_MQTT_Poll()` is only
  called freely while the MQTT session is up. When the session has dropped it
  is called only from the pairing screens (never while a transmitter is
  selected), only when `ModemGate_SimReady()` passes, and at most every
  6 s -> 12 s -> ... -> 60 s. Without this, one dropped session meant
  `fullReconnect()` (network wait + TLS connect, up to a minute or more,
  retried immediately when it fails) running back-to-back: about one loop
  iteration per minute for the whole outage. Consequence: after a drop during
  a live LoRa session the cloud link stays down until the session ends.
- **Known team-layer behaviours left as designed** (protected files):
  `AWS_Init()` calls `AWS_CertStorage_Erase()` whenever the MQTT connect fails
  with a stored certificate — including plain "no coverage" failures — so the
  next attempt re-provisions by claim. `AWS_Manager_Tick()` publishes
  telemetry every 10 s and a heartbeat every 30 s, each a blocking
  ~0.5-1.5 s AT exchange, and on every (re)connect re-subscribes and
  re-registers (a few seconds). `GSM_GetMsSinceLastMessage()` is refreshed by
  telemetry from ANY transmitter, so with several transmitters online the
  cloud path can look alive while the selected one is silent.
- **LoRa Fallback & Pairing Slot Fix**:
  * Double-buffered frame reception (`s_completed_buf`) in `lora_e220.c` to prevent newly arriving bytes from corrupting assembled frames before `LoRa_Poll()` runs.
  * Increased `LORA_PRESENCE_TIMEOUT_MS` from 5s to 15s (matching GSM) so presence does not drop out during blocking modem initialization / AT commands.
  * Optimized rolling window intervals (`LORA_SLOT_INTERVAL_MS = 1000u`, `LORA_ENTER_SUCCESS_COUNT = 3u`) and `ConnectionIsReady()` in `screen_sm.c` for fast, reliable fallback connection when GSM is unavailable.
- **Display Header LoRa Status**:
  * `PushStatusBar()` in `screen_sm.c` now outputs `"LoRa"` on `VP_CONN_MODE` when connected via LoRa, and `"GSM"` when connected via cellular/MQTT.
- **Offline Telemetry Queueing & isLoRa Payload**:
  * Added `OfflineTelemetry_t` FIFO queue (128 capacity) in `aws_manager.c/h`.
  * When disconnected from GSM/AWS (or operating on LoRa), telemetry samples are timestamped (`uptime_s`), tagged with `"isLoRa": true/false`, and stored locally.
  * Upon GSM reconnection, `AWS_Manager_Tick()` automatically drains the offline queue and delivers all stored packets to AWS IoT before continuing normal live telemetry publishing.
