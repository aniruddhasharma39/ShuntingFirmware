 ...
 * ("0002" / "SN-CUSTOM-02"), uncoordinated with device_config.h's own
 * DEVICE_ID/DEVICE_SERIAL_NUMBER. device_config.h is now the single
 * place to change per physical unit before each build. */
...
/* static const char *DEVICE_DESCRIPTION =
    "Receiver_device_deployed_at_Yard_Alpha"; */
        ...
        if (strncmp(topic, prefix, strlen(prefix)) == 0)
        {
            ...
            if (id_end)
            {
                ...
                if (id_len < sizeof(id_str))
                {
                    ...
                    if (id_len == 5 && strncmp(id_str, "TX-", 3) == 0 && 
                        isdigit((unsigned char)id_str[3]) && isdigit((unsigned char)id_str[4]))
                    {
                        uint8_t device_num = (uint8_t)atoi(id_str + 3);
                        if (device_num >= 1 && device_num <= 10)
                        {
                            GSM_MarkDeviceOnline(device_num);
                        }
                        else
                        {
                            printf("[PRESENCE] Invalid Pairing Slot %u from %s rejected.\r\n", device_num, id_str);
                        }
                    }
                    else
                    {
                        printf("[PRESENCE] Non-Transmitter ID '%s' ignored.\r\n", id_str);
                    }
                }
            }
        }
                ...
                if (id_len < sizeof(id_str))
                {
                        ...
                        isdigit((unsigned char)id_str[3]) && isdigit((unsigned char)id_str[4]))
                    {
                        ...
                        if (device_num >= 1 && device_num <= 10)
                        {
                            ...
                            /* Extract distance_cm from payload: "distance_cm":<number> */
                            const char *d_pos = strstr(payload, "\"distance_cm\":");
                            if (d_pos)
                            {
                                d_pos += strlen("\"distance_cm\":");
                                while (*d_pos == ' ' || *d_pos == '\t') d_pos++;
...
static void AWS_RestoreSubscriptions(void)
{
    ...
    /* ----------------------------------------------------
     * Subscribe to peer status (Presence Protocol)
     * ---------------------------------------------------- */
    if (!GSM_MQTT_SubscribeTopic("devices/+/status"))
    {
        printf("[AWS] Peer status subscription failed.\r\n");
    }

    /* ----------------------------------------------------
     * Subscribe to peer telemetry (Distance Protocol)
     * ---------------------------------------------------- */
    if (!GSM_MQTT_SubscribeTopic("devices/+/telemetry"))
    {
        printf("[AWS] Peer telemetry subscription failed.\r\n");
    }
}
...
uint8_t AWS_PublishHeartbeat(void)
{
    ...
    snprintf(
        payload,
        sizeof(payload),
        "{\"deviceId\":\"%s\",\"productType\":\"%s\",\"status\":\"ONLINE\",\"uptime_s\":%lu}",
        DEVICE_ID,
        DEVICE_TYPE_STR,
...
)
{
    ...
    {
        printf("[AWS] MQTT reconnected - restoring subscriptions and registering\r\n");
        AWS_RestoreSubscriptions();
        AWS_RegisterDevice();
        AWS_PublishHeartbeat();
